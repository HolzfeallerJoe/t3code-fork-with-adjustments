...
- **Do not run repo-wide checks.** No `vp check`, no `vp run -r test`, no `vp run -r typecheck` unless I ask. This fork has no CI, so a broken repo-wide run is noise, not a signal.
